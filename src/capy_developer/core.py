    ...
    def _seed_new_project(self, repository: Path, project_id: str, specification: dict) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="seed-", dir=seeds) as temporary_text:
            ...
            with (seed / "CAPY.md").open("a", encoding="utf-8") as instructions:
                instructions.write(
                    "\nIf .capy-local/handoff.json exists, use the configured Capy Developer MCP tools "
                    "for the whole lifecycle. Read its nonsecret handoff_id and invoke the MCP tool "
                    "capy_development_attach before editing; use capy_development_inspect, "
                    "capy_development_verify, capy_release_candidate_create and "
                    "capy_development_finish as needed. These tool names are not shell commands. "
                    "A plain capy-dev command on PATH can use a different installation and catalog "
                    "from the configured MCP server, causing HANDOFF_NOT_FOUND or SESSION_NOT_FOUND. "
                    "Use the configured MCP tools instead of guessing CLI roots or changing config. "
                    "If those tools are unavailable, report that specific problem; do not use the "
                    "website to create another handoff as an attachment repair. The session is "
                    "already prepared; do not call development_start to recreate it. Implement only "
                    "the owner's actual request. Obtain the full commit with git rev-parse HEAD; "
                    "abbreviated hashes are not verification identities. Run Python checks with "
                    "PYTHONDONTWRITEBYTECODE=1 to avoid generated bytecode dirtying the worktree. "
                    "After committing and passing verification, create a candidate and finish "
                    "the session. A candidate is not independently accepted, installed, previewed "
                    "or deployed.\n")
