...
TOOLS = [
    {"name": "capy_connection_contracts", "description": "Discover finite supported managed connection contracts through the configured Capy site. Returns exact request/result schemas and synthetic examples; provider credentials stay managed by Capy. Connection availability is not checked. Use this before authoring a connected app; collect required inputs rather than inventing values.",
     "inputSchema": {"type": "object", "required": ["client_id"], "additionalProperties": False,
                     "properties": {"client_id": {"type": "string", "pattern": "^cli_[0-9a-f]{32}$"},
                                    "contract": {"type": "string", "maxLength": 86}}}},
    ...
    {"name": "capy_client_check", "description": "Perform the configured client's fresh MCP tool challenge through its approved computer connection.",
     "inputSchema": {"type": "object", "required": ["client_id"], "additionalProperties": False,
                     "properties": {"client_id": {"type": "string", "pattern": "^cli_[0-9a-f]{32}$"}}}},
    {"name": "capy_work_begin", "description": "Prepare website-linked work before editing: explicit new app, separate session in an exact registered project, exact completed handoff continuation, or link an existing clean READY session_id without creating a session. Returns the managed workspace and review URL without launching another client.",
     "inputSchema": {"type": "object", "required": ["client_id", "intent_id", "request"], "additionalProperties": False,
       "properties": {"client_id": {"type": "string"}, "intent_id": {"type": "string", "pattern": "^[0-9a-f]{32}$"},
           "request": {"type": "string"}, "session_id": {"type": "string", "minLength": 1}, "parent_handoff_id": {"type": "string"},
       ...
       "description": "Provide exactly one of new, existing, session_id, or parent_handoff_id. The server rejects missing or conflicting intent."}},
