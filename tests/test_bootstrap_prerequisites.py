...
def test_generated_script_is_fixed_and_parses(tmp_path):
    ...
    assert shell is not None, 'POSIX generator syntax qualification requires sh (included with Git on Windows)'
    assert subprocess.run([shell,'-n',str(p)],capture_output=True).returncode==0
