...
def validate(payload: bytes) -> dict:
    ...
    if not isinstance(project["project_id"],str) or re.fullmatch(r"prj_[A-Za-z0-9_]{1,96}",project["project_id"]) is None or not identifier(app["id"]) or not isinstance(verification["verification_id"],str) or re.fullmatch(r"ver_[A-Za-z0-9_]{1,124}",verification["verification_id"]) is None or not isinstance(receipt["session_id"],str) or re.fullmatch(r"ses_[A-Za-z0-9_]{1,124}",receipt["session_id"]) is None or not all(hex40(source[key]) for key in ("commit","tree","base_commit")) or not hex40(tool["release_binding_commit"]) or not hex40(tool["implementation_commit"]): raise ValueError("identity syntax")
    ...
    if tool.get("release_binding_commit") != DEVKIT_COMMIT or tool.get("implementation_commit") != DEVKIT_SOURCE or tool.get("wheel_sha256") != WHEEL_SHA256 or tool.get("wheel_filename") != release.get("wheel_filename") or tool.get("interaction_contract") != INTERACTION_SCHEMA: raise ValueError("manifest toolchain")
